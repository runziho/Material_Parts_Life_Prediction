    ...
    if df is None:
        ...
        if st.button("🚀 예측 시작", type="primary"):
            with st.spinner("예측 중..."):
                ...
                X_ann_s           = ann_scaler_X.transform(X[ALL_COLS])
                ann_mean, ann_std = predict_ann(X_ann_s)

                X_ngb_s           = ngb_scaler_X.transform(X[ALL_COLS])
